...
interface TemplateDetailContentProps {
  template: any; // Sostituisci "any" con l'interfaccia corretta del tuo template
  lang: string;
}

export function TemplateDetailContent({ template, lang }: TemplateDetailContentProps) {
  const sheetRef = useRef<HTMLDivElement>(null);
  const overlayRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Sblocco forzato dell'overflow sul body in caso di residui
    ...
    // 2. Animazione del pannello del template (Slide Up fisso dal basso)
    if (sheetRef.current) {
      gsap.fromTo(
        sheetRef.current,
        { y: "100vh" },
        {
          ...
          duration: 0.5,
          ease: "power3.out",
          clearProps: "transform", // Essenziale per lo scrolling interno
        }
      );
    }
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center sm:p-4">
      ...
      >
        ...
        {/* Contenitore scrollabile del template */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          <div className="p-8 sm:p-12">
            
            {/* REINSERISCI QUI LA LOGICA DEL TUO TEMPLATE */}
            ...
            <p style={{ color: "var(--muted)", fontSize: "18px", marginTop: "16px" }}>
              {template?.description || "Descrizione del template inserita qui."}
            </p>
