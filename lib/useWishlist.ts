...
export function useWishlist() {
  ...
  useEffect(() => {
    ...
    fetch("/api/wishlist")
      ...
      .then((data) => {
        ...
        Promise.all(toSync.map((id) =>
          fetch("/api/wishlist", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ templateId: id }) })
        )).catch((e) => console.error("[useWishlist sync]", e));
        setIds(merged);
        try { localStorage.setItem(KEY, JSON.stringify(merged)); } catch (e) { console.error("[useWishlist]", e); }
        setSynced(true);
      })
      .catch((e) => {
  ...
  const toggle = useCallback((id: string) => {
    setIds((prev) => {
      ...
      try { localStorage.setItem(KEY, JSON.stringify(next)); } catch (e) { console.error("[useWishlist]", e); }
      return next;
    });
